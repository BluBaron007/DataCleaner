...
# -----------------------------
# Force Background to Aqua White
# -----------------------------
st.markdown("""
    <style>
    .stApp {
...
if submitted and ticker and exp_date and chosen_strike:
    ...
    if volatility == 0 or days_to_expiry <= 0:
        st.error("⚠️ Volatility or expiration issue.")
    else:
        ...
        st.write(f"🛡 Minimax (best worst-case): **{minimax_strategy}** (${minimax:.2f})")
        st.write(f"🎯 Expected Value Pick: **{best_ev_strategy}** (${ev[np.argmax(ev)]:.2f})")
